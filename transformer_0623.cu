...
int main(int argc, char* argv[]){
    ...
    if(argc != 6)
    {
        printf("./transformer_fp32_gpu batch_size num_layers seq_len head_num size_per_head\n");
    ...
    cudaMalloc((void **)&q_bias_out, sizeof(VALUE_TYPE)*m*n);
    cudaMalloc((void **)&k_bias_out, sizeof(VALUE_TYPE)*m*n);
    cudaMalloc((void **)&v_bias_out, sizeof(VALUE_TYPE)*m*n);
    ...
    VALUE_TYPE *q_grad, *k_grad, *v_grad, *k_grad_trans;
    cudaMalloc((void **)&q_grad, sizeof(VALUE_TYPE)*n*m);
    cudaMalloc((void **)&k_grad_trans, sizeof(VALUE_TYPE)*n*m);
