...
int main(int argc, char* argv[]){
    ...
    float *q_grad, *k_grad, *v_grad, *k_grad_trans;
    cudaMalloc((void **)&q_grad, sizeof(float)*n*m);
    cudaMalloc((void **)&k_grad_trans, sizeof(float)*n*m);
    ...
    cudaMemset(loss_val, 0.0, sizeof(float)*K);
    ...
    float *temp1 = (float*)malloc(sizeof(float)*m*n*4);
    ...
    for(int iter=0;iter<1;iter++)
    {
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/x_embed/x-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(d_xembed, path1, vocab_size*n);
        
#endif

        cudaMemcpy(temp1, d_xembed, sizeof(float)*vocab_size*n, cudaMemcpyDeviceToHost);
        ele_num = vocab_size*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        }
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("xembed :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/p_embed/p-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(d_pembed, path1, seq_len*n);
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/embed/embed-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(embed_lookup, path1, m*n);
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/dropout-x/dropout-x-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(x_after_dropout, path1, m*n);
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/dropout-p/dropout-p-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(p_after_dropout, path1, seq_len*n);
        ...
        memset(path, 0.0, sizeof(char)*100);
        memset(path1, 0.0, sizeof(char)*100);
        strcpy(path, "./data/input/input-");
        strcpy(path1, path);
        sprintf(temp, "%d", iter);
        strcat(path1, temp);

        gpu_save_float(embed_add, path1, m*n);
        
#endif
        cudaMemcpy(temp1, embed_add, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("input_forward :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(q_w, path1, n*n);

#endif
        cudaMemcpy(temp1, d_qw, sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("q_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(k_w, path1, n*n);

#endif
        cudaMemcpy(temp1, d_kw, sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("k_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(v_w, path1, n*n);

#endif
        cudaMemcpy(temp1, d_vw, sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("v_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(a_w, path1, n*n);

#endif
        cudaMemcpy(temp1, d_aw, sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("a_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(m1_w, path1, n*n*4);

#endif
        cudaMemcpy(temp1, d_m1w, sizeof(float)*n*n*4, cudaMemcpyDeviceToHost);
        ele_num = n*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m1_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
# ifdef SAVE_WEIGHT
            ...
            cpu_save_float(m2_w, path1, n*n*4);

#endif
        cudaMemcpy(temp1, d_m2w, sizeof(float)*n*n*4, cudaMemcpyDeviceToHost);
        ele_num = n*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m2_w :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/norm_a/norm_a-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(norm_a+(i*m*n), path1, m*n);
        
#endif
        cudaMemcpy(temp1, norm_a+(i*m*n), sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("norm_a :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(q_out, path1, m*n);
        
#endif
        cudaMemcpy(temp1, q_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("q_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(k_out, path1, m*n);
        
#endif
        cudaMemcpy(temp1, k_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("k_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(v_out, path1, m*n);
        
#endif
        cudaMemcpy(temp1, v_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("v_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(qk_out, path1, batch_size*head_num*seq_len*seq_len);
        
#endif
        cudaMemcpy(temp1, qk_out, sizeof(float)*batch_size*head_num*seq_len*seq_len, cudaMemcpyDeviceToHost);
        ele_num = batch_size*head_num*seq_len*seq_len;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        }
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("qk_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/softmax/softmax-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(softmax_out+(i*batch_size*head_num*seq_len*seq_len), path1, batch_size*head_num*seq_len*seq_len);
        
#endif
        cudaMemcpy(temp1, softmax_out+(i*batch_size*head_num*seq_len*seq_len), sizeof(float)*batch_size*head_num*seq_len*seq_len, cudaMemcpyDeviceToHost);
        ele_num = batch_size*head_num*seq_len*seq_len;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("softmax :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(sv_out, path1, m*n);
        
#endif
        cudaMemcpy(temp1, sv_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("sv_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(a_out, path1, m*n);
        
#endif
        cudaMemcpy(temp1, a_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("a_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/dropout-a/dropout-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(a_after_dropout, path1, m*n);
        
#endif

        cudaMemcpy(temp1, a_after_dropout, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("a_after_dropout :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
...
//dont haves
            ...
            gpu_save_float(add_1+(i*m*n), path1, m*n);
        
#endif

        cudaMemcpy(temp1, add_1+(i*m*n), sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("add_1 :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/norm_m/norm_m-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(norm_m+(i*m*n), path1, m*n);
        
#endif
        cudaMemcpy(temp1, norm_m+(i*m*n), sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("norm_m :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(m1_out, path1, m*n*4);
        
#endif

        cudaMemcpy(temp1, m1_out, sizeof(float)*m*n*4, cudaMemcpyDeviceToHost);
        ele_num = m*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m1_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/m1_biasgelu/m1-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(m1_gelu_out+(i*m*n*4), path1, m*n*4);
        
#endif

        cudaMemcpy(temp1, m1_gelu_out+(i*m*n*4), sizeof(float)*m*n*4, cudaMemcpyDeviceToHost);
        ele_num = m*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m1_gelu_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(m2_out, path1, m*n);
        
#endif

        cudaMemcpy(temp1, m2_out, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m2_out :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/dropout-m/dropout-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(m_after_dropout, path1, m*n);
        
#endif

        cudaMemcpy(temp1, m_after_dropout, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m_after_dropout :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            gpu_save_float(embed_add+(i+1)*m*n, path1, m*n);
        
#endif

        cudaMemcpy(temp1, embed_add+(i+1)*m*n, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("add2 :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
        ...
        cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N,
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/logits/logits-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);

            gpu_save_float(logits, path1, vocab_size*m);
        ...
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/logits_dx/dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);


            gpu_save_float(logits_dx+(num_layers*m*n), path1, m*n);
        ...
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T,
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/logits_dw/dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);

            gpu_save_float(logits_dw, path1, vocab_size*n);
        ...
        for(int i = num_layers-1; i>=0; i--){
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/dropoutm_grad/dropoutm-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(dx1, path1, n*m);
        
#endif

        cudaMemcpy(temp1, dx1, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("dropout_m_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/m2_grad_dx/m2_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(dx_41, path1, n*m*4);
        
#endif

        cudaMemcpy(temp1, dx_41, sizeof(float)*m*n*4, cudaMemcpyDeviceToHost);
        ele_num = m*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m2_grad_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/m2_grad_dw/m2_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_m2dw+(i*n*n*4), path1, n*n*4);
        
#endif

        cudaMemcpy(temp1, d_m2dw+(i*n*n*4), sizeof(float)*n*n*4, cudaMemcpyDeviceToHost);
        ele_num = n*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m2_grad_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/m1_grad_dx/m1_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m1_grad_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/m1_grad_dw/m1_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_m1dw+(i*n*n*4), path1, n*n*4);
        
#endif

        cudaMemcpy(temp1, d_m1dw+(i*n*n*4), sizeof(float)*n*n*4, cudaMemcpyDeviceToHost);
        ele_num = n*n*4;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("m1_grad_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/norm_m_grad/normm_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(dx2, path1, m*n);
        
#endif

        cudaMemcpy(temp1, dx2, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("norm_m_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/grad_add1/add1-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(grad_add1, path1, m*n);
        
#endif

        cudaMemcpy(temp1, grad_add1, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("grad_add1 :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/dropouta_grad/dropouta-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("dropout_a_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/a_grad_dx/a_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("a_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/a_grad_dw/a_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_adw+(i*n*n), path1, n*n);
        
#endif


        cudaMemcpy(temp1, d_adw+(i*n*n), sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("a_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/sv_grad_dx/sv_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(sv_grad, path1, batch_size*head_num*seq_len*seq_len);
        
#endif

        cudaMemcpy(temp1, sv_grad, sizeof(float)*batch_size*head_num*seq_len*seq_len, cudaMemcpyDeviceToHost);
        ele_num = batch_size*head_num*seq_len*seq_len;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("sv_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/softmax_grad/softmax_grad-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(softmaxgrad, path1, batch_size*head_num*seq_len*seq_len);
        
#endif

        cudaMemcpy(temp1, softmaxgrad, sizeof(float)*batch_size*head_num*seq_len*seq_len, cudaMemcpyDeviceToHost);
        ele_num = batch_size*head_num*seq_len*seq_len;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("softmaxgrad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            transpose_0123to0132_gpu<<<(batch_size*head_num*seq_len), size_per_head>>>(k_grad, k_grad_trans, batch_size, seq_len, head_num, size_per_head);

            transpose_0213to0123_gpu<<<(batch_size*seq_len*head_num), size_per_head>>>(q_grad, q_grad_reshape, batch_size, seq_len, head_num, size_per_head);
            transpose_0213to0123_gpu<<<(batch_size*seq_len*head_num), size_per_head>>>(k_grad_trans, k_grad_reshape, batch_size, seq_len, head_num, size_per_head);
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/q_grad/q_grad-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(q_grad_reshape, path1, m*n);
        
#endif

        cudaMemcpy(temp1, q_grad_reshape, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("q_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/k_grad/k_grad-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(k_grad_reshape, path1, m*n);
        
#endif
        cudaMemcpy(temp1, k_grad_reshape, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("k_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/v_grad/v_grad-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(v_grad_reshape, path1, m*n);
        
#endif

        cudaMemcpy(temp1, v_grad_reshape, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("v_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("v_grad_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/v_grad_dx/v_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
...
#endif
        cudaMemcpy(temp1, d_vdw+(i*n*n), sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("v_grad_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/v_grad_dw/v_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_vdw+(i*n*n), path1, n*n);
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/k_grad_dx/k_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("k_grad_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/k_grad_dw/k_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_kdw+(i*n*n), path1, n*n);
        
#endif

        cudaMemcpy(temp1, d_kdw+(i*n*n), sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("k_grad_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/q_grad_dx/q_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(dx3, path1, m*n);
        
#endif

        cudaMemcpy(temp1, dx3, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("q_grad_dx :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/q_grad_dw/q_dw-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(d_qdw+(i*n*n), path1, n*n);
        
#endif

        cudaMemcpy(temp1, d_qdw+(i*n*n), sizeof(float)*n*n, cudaMemcpyDeviceToHost);
        ele_num = n*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("q_grad_dw :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/grad_add2/add2-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(v_grad, path1, m*n);
        
#endif

        cudaMemcpy(temp1, v_grad, sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("grad_add2 :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            
            layernorm_dg_db_gpu<<<seq_len, n>>>(d_norm_adg+(i*n), d_norm_adb+(i*n), v_grad, embed_add+(i*m*n), norm_a_mean+(i*m), norm_a_rstd+(i*m), batch_size, seq_len, state);
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/norm_a_grad/norma_dx-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("norm_a_grad :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
            ...
            memset(path, 0.0, sizeof(char)*100);
            memset(path1, 0.0, sizeof(char)*100);
            strcpy(path, "./data/grad_add3/add3-");
            strcpy(path1, path);
            sprintf(temp, "%d", iter);
            strcat(path1, temp);
            sprintf(temp, "%d", i);
            ...
            gpu_save_float(logits_dx+(i*m*n), path1, m*n);
        
#endif

        cudaMemcpy(temp1, logits_dx+(i*m*n), sizeof(float)*m*n, cudaMemcpyDeviceToHost);
        ele_num = m*n;
        for(int j=0;j<ele_num;j++){
            if(temp1[j] != 0)
                nonzero_num++;
        ...
        sparsity=(float)nonzero_num/(float)ele_num;
        printf("grad_add3 :: ele_num :%d, nonzero_num :%d sparsity :%f\n", ele_num, nonzero_num, sparsity);
        nonzero_num = 0;
        ...
        lr = (iter+1) * (1.0/1000) < 1 ? (iter+1) * (1.0/1000) : 1;
        lr *= learning_rate;

        beta1_power = adam_got_beta_power(beta1, (iter+1));
        beta2_power = adam_got_beta_power(beta2, (iter+1));
