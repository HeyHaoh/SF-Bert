...
template <bool K64>
__global__ void __launch_bounds__(256,3) bst_sgemm_32x32x64_nt_float(
    ...
    uint szHeadBlocksBlk, uint szBlocksBlk, uint szLut, uint loops)
{
    ...
    *(float4*)&fShare[storC + 3*32*8] = *(float4*)regC[7];
    ...
    store((float2*)(C+16*32), c2[0]);

}

bool bst_sgemm_nt_float(CUstream stream,
    const uint2* lut,
    const float* a,
    const float* b,
          float* c,
    ...
    uint lut_heads, uint lut_dim)
{
    ...
    {
        if (k64)
            bst_sgemm_32x32x64_nt_float< true><<<grid,256,0,stream>>>(lut, a, b, c, szCtxHeadStateA, szCtxHeadStateB, szHeadState, szState, szHeadBlocksBlk, szBlocksBlk, szLut, loops);
        else
            bst_sgemm_32x32x64_nt_float<false><<<grid,256,0,stream>>>(lut, a, b, c, szCtxHeadStateA, szCtxHeadStateB, szHeadState, szState, szHeadBlocksBlk, szBlocksBlk, szLut, loops);
    }
    return true;
}
    ...
    uint lut_heads, uint lut_dim, uint op, uint magic, uint shift, uint max_lut)
{
    ...
    // printf("%d %d %d ", gridX, batch_dim, head_dim);  
...
template <bool K64>
__global__ void __launch_bounds__(256,3) bst_sgemm_32x32x64_nt_nos(
    const uint2* __restrict__ Lut,
    const float* __restrict__ A,
    const float* __restrict__ B,
          bhalf*              C,
...
bool bst_sgemm_nt_nosl(CUstream stream,
    const uint2* lut,
    const float* a,
    const float* b,
          bhalf* c,
    ...
    uint lut_heads, uint lut_dim)
{
    ...
    {
        if (k64)
            bst_sgemm_32x32x64_nt_nos< true><<<grid,256,0,stream>>>(lut, a, b, c, szCtxHeadStateA, szCtxHeadStateB, szHeadState, szState, szHeadBlocksBlk, szBlocksBlk, szLut, loops);
        else
            bst_sgemm_32x32x64_nt_nos<false><<<grid,256,0,stream>>>(lut, a, b, c, szCtxHeadStateA, szCtxHeadStateB, szHeadState, szState, szHeadBlocksBlk, szBlocksBlk, szLut, loops);
    }
    return true;
}
...
template <uint UNROLL, uint BLOCKS, uint BSIZE, typename T, typename V2, typename MASKT>
__global__ void __launch_bounds__(1024,BLOCKS) bst_masked_softmax_nol(
    const uint2* __restrict__ Lut,
    const MASKT* __restrict__ Mask,
    const bhalf* __restrict__ X,
              T*              Y,
...
template <typename T, typename V>
bool BlocksparseMaskedSoftmax_nosl(CUstream stream,
    const uint2* lut,
    const  uint* mask,
    const bhalf* x,
              T* y,
    ...
    uint mask_heads, float scale)
{
    ...
    if (block_size == 64)
    {
        shared = lut_max * 12;
            if (unroll == 16)
            bst_masked_softmax_nol<16,1,64,T,V,uint64><<<grid,threads,shared,stream>>>(lut, (const uint64*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else if (unroll == 8)
            bst_masked_softmax_nol< 8,2,64,T,V,uint64><<<grid,threads,shared,stream>>>(lut, (const uint64*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else // (unroll == 4)
            bst_masked_softmax_nol< 4,2,64,T,V,uint64><<<grid,threads,shared,stream>>>(lut, (const uint64*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
    }
    else if (block_size == 32)
    {
            if (unroll == 16)
            bst_masked_softmax_nol<16,1,32,T,V,  uint><<<grid,threads,shared,stream>>>(lut, (const   uint*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else if (unroll == 8)
            bst_masked_softmax_nol< 8,2,32,T,V,  uint><<<grid,threads,shared,stream>>>(lut, (const   uint*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else // (unroll == 4)
            bst_masked_softmax_nol< 4,2,32,T,V,  uint><<<grid,threads,shared,stream>>>(lut, (const   uint*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
    }
    else if (block_size == 16)
    {
            if (unroll == 16)
            bst_masked_softmax_nol<16,1,16,T,V,ushort><<<grid,threads,shared,stream>>>(lut, (const ushort*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else if (unroll == 8)
            bst_masked_softmax_nol< 8,2,16,T,V,ushort><<<grid,threads,shared,stream>>>(lut, (const ushort*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else // (unroll == 4)
            bst_masked_softmax_nol< 4,2,16,T,V,ushort><<<grid,threads,shared,stream>>>(lut, (const ushort*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
    }
    else
    {
            if (unroll == 16)
            bst_masked_softmax_nol<16,1, 8,T,V, uchar><<<grid,threads,shared,stream>>>(lut, (const  uchar*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else if (unroll == 8)
            bst_masked_softmax_nol< 8,2, 8,T,V, uchar><<<grid,threads,shared,stream>>>(lut, (const  uchar*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
        else // (unroll == 4)
            bst_masked_softmax_nol< 4,2, 8,T,V, uchar><<<grid,threads,shared,stream>>>(lut, (const  uchar*)mask, x, y, blocks, szLut, szMask, szHead, szBatch, scale, shfl_init, lut_max, mask != NULL);
    }
    return true;
};
